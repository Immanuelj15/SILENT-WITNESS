    ...
    <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
        ...
        <div className="space-y-2 bg-slate-950/60 p-3 rounded-lg border border-slate-800/80">
          <div>
            <span className="text-slate-400 block text-[11px]">Claimed Role / Persona:</span>
          ...
          <div>
            <span className="text-slate-400 block text-[11px]">Claimed Organization:</span>
          ...
          <div className="pt-1">
            <span className="text-[11px] text-slate-400">Official Directory Policy:</span>
        ...
        <div className="flex flex-col justify-between bg-slate-950/60 p-3 rounded-lg border border-slate-800/80">
          <div>
            <span className="text-slate-400 block text-[11px] mb-1">Behavioral Audit Findings:</span>
            {contradictions.length > 0 ? (
              <div className="space-y-1.5">
                {contradictions.map((c, i) => (
                  <div key={i} className="text-red-300 bg-red-950/30 p-2 rounded border border-red-900/50 leading-relaxed">
                    ⚠ {c}
                  </div>
                ))}
              </div>
          ...
          <div className="mt-3 pt-2 border-t border-slate-800/80 flex items-center justify-between">
            <button
              ...
              className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded text-xs font-medium transition-all shadow-sm flex items-center gap-1.5"
            >
              <PhoneCall className="w-3.5 h-3.5" />
              Verify Official Contact
            </button>
            <span className="text-[11px] text-slate-500">Zero Trust Protocol</span>
          </div>
        </div>
      </div>
